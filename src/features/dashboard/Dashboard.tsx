...
export function Dashboard({ user, role }: { user: Pick<User, 'email'>; role: 'admin' | 'superAdmin' }) {
  const [section, setSection] = useState<'overview' | 'subjects' | 'exams' | 'users' | 'courses' | 'tests' | 'profile'>('overview')
  const [profileOpen, setProfileOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false)
  const [openTopicId, setOpenTopicId] = useState<string | null>(null)
  const clearOpenTopicId = useCallback(() => setOpenTopicId(null), [])
  ...
  return <div className={`layout ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    <aside><p className="eyebrow sidebar-label">SMARTEDU</p><h2 className="sidebar-label">Admin Console</h2><nav>
      {([['overview', '⌂', 'Overview'], ...(role === 'superAdmin' ? [['users', '♙', 'Manage Users'] as const] : []), ['subjects', '▦', 'Subjects'] as const, ['exams', '▤', 'Exams'] as const, ['tests', '✓', 'Test Series']] as const).map(([key, icon, label]) =>
    ...
    <main className="content"><div className="topbar"><div className="brand"><button className="sidebar-toggle" onClick={() => setSidebarCollapsed((v) => !v)} aria-label={sidebarCollapsed ? 'Expand side menu' : 'Collapse side menu'}>{sidebarCollapsed ? '☰' : '‹'}</button><span className="brand-mark">S</span><strong>SmartEdu</strong><span className="brand-banner">Admin learning center</span></div>
      ...
      {section === 'overview' && <Overview />}{section === 'subjects' && <Subjects role={role} openTopicId={openTopicId} onOpenTopicHandled={clearOpenTopicId} />}{section === 'exams' && <Exams onSelectExam={(topicId) => { setOpenTopicId(topicId); setSection('subjects') }} />}{section === 'users' && role === 'superAdmin' && <Users />}{section === 'courses' && <Courses />}{section === 'tests' && <Tests />}
